...
# # === VIDEO WRITER SETUP ===
# fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Hoặc 'XVID' nếu không chạy được
...
# # Ghi lại thời điểm bắt đầu ghi video
# video_start_time = datetime.now()
# video_filename = f"recordings/video_{video_start_time.strftime('%Y%m%d_%H%M%S')}.mp4"

# # Tạo folder recordings nếu chưa có
# import os
...
#     cv2.putText(bgr_image, timestamp_str, (10, frame_height - 10), 
...
# print(f"🕒 Ghi chú thời gian bắt đầu video: {video_start_time}")
...
def record_video(detector):
    ...
    with graph.as_default():
        ...
        # === VIDEO WRITER SETUP ===
        # fourcc = cv2.VideoWriter_fourcc(*'XVID')  # Hoặc 'XVID' nếu không chạy được
        # fps = int(cap.get(cv2.CAP_PROP_FPS))
        # # fps = 20
        # print(f"FPS: {fps}")
        # frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
...
video_thread = threading.Thread(target=record_video, args=(detector,))
